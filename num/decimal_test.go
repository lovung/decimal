...
func TestNewDecimalFromFloatRandom(t *testing.T) {
	n := 0
	...
	for {
		...
		want, err := NewDecimalFromString(strconv.FormatFloat(in, 'f', -1, 64))
		if err != nil {
			t.Error(err)
			continue
		}
		got := NewDecimalFromFloat(in)
		if !want.Equal(got) {
...
func TestNewDecimalFromFloatQuick(t *testing.T) {
	err := quick.Check(func(f float64) bool {
		want, werr := NewDecimalFromString(strconv.FormatFloat(f, 'f', -1, 64))
		if werr != nil {
			return true
		}
		got := NewDecimalFromFloat(f)
...
func TestNewDecimalFromFloat32Random(t *testing.T) {
	n := 0
	rng := rand.New(rand.NewSource(0xdead1337))
	for {
		n++
		...
		want, err := NewDecimalFromString(strconv.FormatFloat(float64(in), 'f', -1, 32))
		if err != nil {
			t.Error(err)
			continue
		}
		got := NewDecimalFromFloat32(in)
		if !want.Equal(got) {
			t.Errorf("in: %v, expected %s (%s, %d), got %s (%s, %d) ",
				in, want.String(), want.value.String(), want.exp,
				got.String(), got.value.String(), got.exp)
...
func TestNewDecimalFromFloat32Quick(t *testing.T) {
	err := quick.Check(func(f float32) bool {
		want, werr := NewDecimalFromString(strconv.FormatFloat(float64(f), 'f', -1, 32))
		if werr != nil {
			return true
		}
		got := NewDecimalFromFloat32(f)
		return got.Equal(want)
	}, &quick.Config{})
	if err != nil {
		t.Error(err)
...
func TestNewDecimalFromFormattedString(t *testing.T) {
	for _, testCase := range []struct {
		...
		dFormatted, err := NewDecimalFromFormattedString(testCase.Formatted, testCase.ReplRegex)
		if err != nil {
			t.Fatal(err)
		}

		dExact, err := NewDecimalFromString(testCase.Expected)
		if err != nil {
			t.Fatal(err)
		}
...
func TestNewDecimalFromStringErrs(t *testing.T) {
	tests := []string{
		"",
		"qwert",
		"-",
...
func TestNewDecimalFromStringDeepEquals(t *testing.T) {
	type StrCmp struct {
		str1     string
		str2     string
		expected bool
	...
	for _, cmp := range tests {
		d1, err1 := NewDecimalFromString(cmp.str1)
		d2, err2 := NewDecimalFromString(cmp.str2)
...
func TestNewDecimalFromInt32(t *testing.T) {
...
func TestNewDecimalFromBigIntWithExponent(t *testing.T) {
	type Inp struct {
		val *big.Int
		exp int32
	}
	...
	for input, s := range tests {
		d := NewDecimalFromBigInt(input.val, input.exp)
		if d.String() != s {
			t.Errorf("expected %s, got %s (%s, %d)",
				s, d.String(),
				d.value.String(), d.exp)
...
func TestDecimal_Shift(t *testing.T) {
	...
	for inp, expectedStr := range inputs {
		num, _ := NewDecimalFromString(inp.a)
					...
					for prec = -n; prec <= n; prec++ { // 2n+1
						for _, v1 := range a { // 11
							for _, v2 := range a { // 11, even if 0 is skipped
								sign1 := NewDecimal(int64(s), 0)
								sign2 := NewDecimal(int64(s2), 0)
								d := sign1.Mul(NewDecimal(int64(v1), e1))
								d2 := sign2.Mul(NewDecimal(int64(v2), e2))
...
func (d Decimal) DivOld(d2 Decimal, prec int) Decimal {
	...
	ret, err := NewDecimalFromString(quoRat.FloatString(prec))
	if err != nil {
...
func TestDecimal_RoundCash(t *testing.T) {
	...
	for i, test := range tests {
		d, _ := NewDecimalFromString(test.d)
		haveRounded := d.RoundCash(test.interval)
		result, _ := NewDecimalFromString(test.result)
...
func TestDecimal_Overflow(t *testing.T) {
	if !didPanic(func() { NewDecimal(1, math.MinInt32).Mul(NewDecimal(1, math.MinInt32)) }) {
		t.Fatalf("should have gotten an overflow panic")
	}
	if !didPanic(func() { NewDecimal(1, math.MaxInt32).Mul(NewDecimal(1, math.MaxInt32)) }) {
		t.Fatalf("should have gotten an overflow panic")
	}
}
...
func TestDecimal_ExtremeValues(t *testing.T) {
	...
	test(func() {
		got := NewDecimal(123, math.MinInt32).Floor()
		if !got.Equal(NewDecimalFromFloat(0)) {
			t.Errorf("Error: got %s, expected 0", got)
		}
	})
	test(func() {
		got := NewDecimal(123, math.MinInt32).Ceil()
		if !got.Equal(NewDecimalFromFloat(1)) {
			t.Errorf("Error: got %s, expected 1", got)
		}
	})
	test(func() {
		got := NewDecimal(123, math.MinInt32).Rat().FloatString(10)
...
func TestDecimal_Min(t *testing.T) {
	...
	for _, test := range testCases {
		...
		expectedDecimal := NewDecimalFromFloat(expected)
		decimalInput := []Decimal{}
		for _, inp := range input {
			d := NewDecimalFromFloat(inp)
...
func TestDecimal_Scan(t *testing.T) {
	...
	a := Decimal{}
	dbvalue := 54.33
	expected := NewDecimalFromFloat(dbvalue)
	...
	expected = NewDecimalFromFloat(float64(dbvalueFloat32))
	...
	dbvalueInt := int64(0)
	expected = NewDecimal(dbvalueInt, 0)
	...
	expected, err = NewDecimalFromString(valueStr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestNullDecimal_Value(t *testing.T) {
	...
	// check that normal case is handled appropriately
	a := NullDecimal{Decimal: NewDecimal(1234, -2), Valid: true}
	expected := "12.34"
	value, err = a.Value()
	if err != nil {
...
func TestSum(t *testing.T) {
	...
	for key := range vals {
		vals[key] = NewDecimal(i, 0)
		i++
	}

	sum := Sum(vals[0], vals[1:]...)
	if !sum.Equal(NewDecimal(45, 0)) {
		t.Errorf("Failed to calculate sum, expected %s got %s", NewDecimal(45, 0), sum)
	}
}
...
func TestAvg(t *testing.T) {
	...
	if !avg.Equal(NewDecimalFromFloat(4.5)) {
		t.Errorf("Failed to calculate average, expected %s got %s", NewDecimalFromFloat(4.5).String(), avg.String())
	}
}
...
func TestAtan(t *testing.T) {
	...
	for i, inp := range inps {
		d, err := NewDecimalFromString(inp)
		if err != nil {
			t.FailNow()
		}
		s, err := NewDecimalFromString(sols[i])
		if err != nil {
			t.FailNow()
		}
		a := d.Atan()
...
func ExampleNewDecimalFromFloat32() {
...
func ExampleNewDecimalFromFloat() {
