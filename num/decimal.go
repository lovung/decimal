...
//
// The best way to create a new Decimal is to use decimal.NewDecimalFromString, ex:
//
//     n, err := decimal.NewDecimalFromString("-123.4567")
...
// NewDecimalFromFormattedString returns a new Decimal from a formatted string representation.
...
func NewDecimalFromFormattedString(value string, replRegexp *regexp.Regexp) (Decimal, error) {
	...
	d, err := NewDecimalFromString(parsedValue)
	if err != nil {
		return Decimal{}, err
	}
	return d, nil
...
// or panics if NewDecimalFromString would have returned an error.
//
// Example:
//
...
func newFromFloat(val float64, bits uint64, flt *floatInfo) Decimal {
	...
	return NewDecimalFromFloatWithExponent(val, int32(d.dp)-int32(d.nd))
}
...
func (d Decimal) Pow(d2 Decimal) Decimal {
	var temp Decimal
	if d2.IntPart() == 0 {
		return NewDecimalFromFloat(1)
	}
	temp = d.Pow(d2.Div(NewDecimalFromFloat(2)))
	if d2.IntPart()%2 == 0 {
...
func Avg(first Decimal, rest ...Decimal) Decimal {
	count := NewDecimal(int64(len(rest)+1), 0)
	sum := Sum(first, rest...)
...
func (d Decimal) Atan() Decimal {
	if d.Equal(NewDecimalFromFloat(0.0)) {
		return d
	}
	if d.GreaterThan(NewDecimalFromFloat(0.0)) {
		return d.satan()
	}
...
func (d Decimal) xatan() Decimal {
	P0 := NewDecimalFromFloat(-8.750608600031904122785e-01)
	P1 := NewDecimalFromFloat(-1.615753718733365076637e+01)
	P2 := NewDecimalFromFloat(-7.500855792314704667340e+01)
	P3 := NewDecimalFromFloat(-1.228866684490136173410e+02)
	P4 := NewDecimalFromFloat(-6.485021904942025371773e+01)
	Q0 := NewDecimalFromFloat(2.485846490142306297962e+01)
	Q1 := NewDecimalFromFloat(1.650270098316988542046e+02)
	Q2 := NewDecimalFromFloat(4.328810604912902668951e+02)
	Q3 := NewDecimalFromFloat(4.853903996359136964868e+02)
	Q4 := NewDecimalFromFloat(1.945506571482613964425e+02)
	z := d.Mul(d)
...
func (d Decimal) satan() Decimal {
	Morebits := NewDecimalFromFloat(6.123233995736765886130e-17) // pi/2 = PIO2 + Morebits
	Tan3pio8 := NewDecimalFromFloat(2.41421356237309504880)      // tan(3*pi/8)
	pi := NewDecimalFromFloat(3.14159265358979323846264338327950288419716939937510582097494459)

	if d.LessThanOrEqual(NewDecimalFromFloat(0.66)) {
		return d.xatan()
	}
	if d.GreaterThan(Tan3pio8) {
		return pi.Div(NewDecimalFromFloat(2.0)).Sub(NewDecimalFromFloat(1.0).Div(d).xatan()).Add(Morebits)
	}
	return pi.Div(NewDecimalFromFloat(4.0)).Add((d.Sub(NewDecimalFromFloat(1.0)).Div(d.Add(NewDecimalFromFloat(1.0)))).xatan()).Add(NewDecimalFromFloat(0.5).Mul(Morebits))
}

// sin coefficients
var _sin = [...]Decimal{
	NewDecimalFromFloat(1.58962301576546568060e-10), // 0x3de5d8fd1fd19ccd
	NewDecimalFromFloat(-2.50507477628578072866e-8), // 0xbe5ae5e5a9291f5d
	NewDecimalFromFloat(2.75573136213857245213e-6),  // 0x3ec71de3567d48a1
	NewDecimalFromFloat(-1.98412698295895385996e-4), // 0xbf2a01a019bfdf03
	NewDecimalFromFloat(8.33333333332211858878e-3),  // 0x3f8111111110f7d0
	NewDecimalFromFloat(-1.66666666666666307295e-1), // 0xbfc5555555555548
}

// Sin returns the sine of the radian argument x.
func (d Decimal) Sin() Decimal {
	PI4A := NewDecimalFromFloat(7.85398125648498535156e-1)                             // 0x3fe921fb40000000, Pi/4 split into three parts
	PI4B := NewDecimalFromFloat(3.77489470793079817668e-8)                             // 0x3e64442d00000000,
	PI4C := NewDecimalFromFloat(2.69515142907905952645e-15)                            // 0x3ce8469898cc5170,
	M4PI := NewDecimalFromFloat(1.273239544735162542821171882678754627704620361328125) // 4/pi
	...
	sign := false
	if d.LessThan(NewDecimalFromFloat(0.0)) {
		d = d.Neg()
		sign = true
	}

	j := d.Mul(M4PI).IntPart()           // integer part of x/(Pi/4), as integer for tests on the phase angle
	y := NewDecimalFromFloat(float64(j)) // integer part of x/(Pi/4), as float

	// map zeros to origin
	if j&1 == 1 {
		j++
		y = y.Add(NewDecimalFromFloat(1.0))
	}
	...
	if j == 1 || j == 2 {
		...
		y = NewDecimalFromFloat(1.0).Sub(NewDecimalFromFloat(0.5).Mul(zz)).Add(w)
	} else {
...
// cos coefficients
var _cos = [...]Decimal{
	NewDecimalFromFloat(-1.13585365213876817300e-11), // 0xbda8fa49a0861a9b
	NewDecimalFromFloat(2.08757008419747316778e-9),   // 0x3e21ee9d7b4e3f05
	NewDecimalFromFloat(-2.75573141792967388112e-7),  // 0xbe927e4f7eac4bc6
	NewDecimalFromFloat(2.48015872888517045348e-5),   // 0x3efa01a019c844f5
	NewDecimalFromFloat(-1.38888888888730564116e-3),  // 0xbf56c16c16c14f91
	NewDecimalFromFloat(4.16666666666665929218e-2),   // 0x3fa555555555554b
}
...
var _tanP = [...]Decimal{
	NewDecimalFromFloat(-1.30936939181383777646e+4), // 0xc0c992d8d24f3f38
	NewDecimalFromFloat(1.15351664838587416140e+6),  // 0x413199eca5fc9ddd
	NewDecimalFromFloat(-1.79565251976484877988e+7), // 0xc1711fead3299176
}
var _tanQ = [...]Decimal{
	NewDecimalFromFloat(1.00000000000000000000e+0),
	NewDecimalFromFloat(1.36812963470692954678e+4),  //0x40cab8a5eeb36572
	NewDecimalFromFloat(-1.32089234440210967447e+6), //0xc13427bc582abc96
	NewDecimalFromFloat(2.50083801823357915839e+7),  //0x4177d98fc2ead8ef
	NewDecimalFromFloat(-5.38695755929454629881e+7), //0xc189afe03cbe5a31
}
...
func (d Decimal) Tan() Decimal {
	...
	if zz.GreaterThan(NewDecimalFromFloat(1e-14)) {
	...
	if j&2 == 2 {
		y = NewDecimalFromFloat(-1.0).Div(y)
	}
	if sign {
		y = y.Neg()
	}
