...
func BenchmarkNewDecimalFromFloatWithExponent(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		_ = NewDecimalFromFloatWithExponent(in, math.MinInt32)
	}
}
...
func BenchmarkNewDecimalFromFloat(b *testing.B) {
	...
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		_ = NewDecimalFromFloat(in[i])
	}
}

func BenchmarkNewDecimalFromStringFloat(b *testing.B) {
	rng := rand.New(rand.NewSource(0xdead1337))
	in := make([]float64, b.N)
	for i := range in {
		in[i] = rng.NormFloat64() * 10e20
	...
	for i := 0; i < b.N; i++ {
		...
		_, _ = NewDecimalFromString(in)
	}
}

func Benchmark_FloorFast(b *testing.B) {
	input := NewDecimal(200, 2)
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		input.Floor()
	}
...
func Benchmark_Cmp(b *testing.B) {
	...
	for i := 0; i < 1000000; i++ {
		decimals = append(decimals, NewDecimal(int64(i), 0))
	}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		sort.Sort(decimals)
...
func Benchmark_decimal_Decimal_Add_different_precision(b *testing.B) {
	d1 := NewDecimalFromFloat(1000.123)
	d2 := NewDecimalFromFloat(500).Mul(NewDecimalFromFloat(0.12))
...
func Benchmark_decimal_Decimal_Add_same_precision(b *testing.B) {
	d1 := NewDecimalFromFloat(1000.123)
	d2 := NewDecimalFromFloat(500.123)
